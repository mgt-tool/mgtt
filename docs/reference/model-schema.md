...
| `failure_modes` | no | Per-component override of how a state propagates: a map of `<state>` → `can_cause: [<label>]`. Feeds the scenario enumerator (which chains a state's `can_cause` labels against downstream `state.triggered_by`). Use to declare propagation the provider type doesn't already express. Example: `failure_modes: { degraded: { can_cause: [upstream_5xx] } }`. |
...
depends:
  - on: api
  - on: vault
    while: vault.state == "starting"   # only depends on vault during boot
```
