...
func TestInstallProvider_RegistryDisabledWrapsSentinel(t *testing.T) {
	...
	var buf bytes.Buffer
	err := installProvider(&buf, "phantom-provider-name-that-cannot-exist", installOpts{})
	if err == nil {
...
func TestInstallProvider_RegistryFlagOverridesEnv(t *testing.T) {
	...
	var buf bytes.Buffer
	err := installProvider(&buf, "phantom", installOpts{Registry: "disabled"})
	if !errors.Is(err, registry.ErrRegistryDisabled) {
	...
	fakeDocker := &providersupport.DockerCmd{
		Run: func(_ context.Context, args ...string) ([]byte, error) {
			...
			case "cp":
				...
				// /types missing → no types. DockerCmd.ExtractTypes treats a
				// docker-absent-source cp error (stderr contains "No such
				// container:path") as "no types" and surfaces anything else.
				return []byte("Error: No such container:path: cid-test:/types"), errors.New("exit status 1")
			case "rm":
				return nil, nil
			}
			return nil, nil
...
`
	...
	var buf bytes.Buffer
	err := installProvider(&buf, srcDir, installOpts{})
	if err == nil {
