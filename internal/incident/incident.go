// Copyright (C) 2026 Alex Kunich
// SPDX-License-Identifier: AGPL-3.0-or-later

package incident

import (
	"fmt"
	"os"
	"strings"
	"sync/atomic"
	"time"

	"github.com/mgt-tool/mgtt/internal/facts"
)

// idSeq breaks ties between generateID calls in the same microsecond.
// Process-local atomic counter: if two starts happen to land in the
// same µs, the second gets suffix+1.
var idSeq atomic.Uint64

// Incident represents an active or completed incident session.
type Incident struct {
	ID        string
	Model     string
	Version   string
	Started   time.Time
	Ended     time.Time // zero until End is called
	Verdict   string
	ModelRef  string // absolute path, empty for CLI-started
	StateFile string
	Store     *facts.Store
}

const currentFile = ".mgtt-current"

// Start creates a new incident for the CLI path: refuses if another is
// active (single-active invariant) and writes the `.mgtt-current` pointer
// so subsequent CLI commands see it.
func Start(modelName, modelVersion, id string) (*Incident, error) {
	if _, err := Current(); err == nil {
		return nil, fmt.Errorf("incident already in progress — run 'mgtt incident end' first")
	}
	inc, err := StartIsolated(modelName, modelVersion, id, "", nil)
	if err != nil {
		return nil, err
	}
	if err := os.WriteFile(currentFile, []byte(inc.StateFile+"\n"), 0644); err != nil {
		return nil, fmt.Errorf("writing current pointer: %w", err)
	}
	return inc, nil
}

// StartIsolated creates a new incident WITHOUT writing `.mgtt-current` and
// WITHOUT checking for another active incident. Used by the MCP path where
// agents may drive multiple concurrent incidents in one `$MGTT_HOME`.
// modelRef, when non-empty, is persisted so downstream tools can re-load
// the model without the caller passing the path again. suspects is the
// agent's initial suspicion list (raw "component" or "component.state").
func StartIsolated(modelName, modelVersion, id, modelRef string, suspects []string) (*Incident, error) {
	if id == "" {
		id = generateID()
	}
	now := time.Now()
	stateFile := id + ".state.yaml"

	meta := facts.StoreMeta{
		Model:    modelName,
		Version:  modelVersion,
		Incident: id,
		Started:  now,
		ModelRef: modelRef,
		Suspects: suspects,
	}
	store := facts.NewDiskBacked(stateFile, meta)
	if err := store.Save(); err != nil {
		return nil, fmt.Errorf("creating state file: %w", err)
	}
	return &Incident{
		ID:        id,
		Model:     modelName,
		Version:   modelVersion,
		Started:   now,
		ModelRef:  modelRef,
		StateFile: stateFile,
		Store:     store,
	}, nil
}

// End closes the current incident: persists the end timestamp (via the
// same code path as EndByID so CLI-ended and MCP-ended incidents are
// indistinguishable on reload) and removes the .mgtt-current pointer.
func End() (*Incident, error) {
	inc, err := Current()
	if err != nil {
		return nil, fmt.Errorf("no active incident: %w", err)
	}
	ended, err := EndByID(inc.ID, "")
	if err != nil {
		return nil, err
	}
	if err := os.Remove(currentFile); err != nil && !os.IsNotExist(err) {
		return nil, fmt.Errorf("remove current pointer: %w", err)
	}
	return ended, nil
}

// LoadByID loads an incident state file by its id without consulting
// `.mgtt-current`. Used by the MCP path where agents hold incident IDs
// explicitly and multiple incidents coexist.
func LoadByID(id string) (*Incident, error) {
	stateFile := id + ".state.yaml"
	store, err := facts.Load(stateFile)
	if err != nil {
		return nil, fmt.Errorf("loading state for %q: %w", id, err)
	}
	return fromStore(store, stateFile), nil
}

func fromStore(store *facts.Store, stateFile string) *Incident {
	return &Incident{
		ID:        store.Meta.Incident,
		Model:     store.Meta.Model,
		Version:   store.Meta.Version,
		Started:   store.Meta.Started,
		Ended:     store.Meta.Ended,
		Verdict:   store.Meta.Verdict,
		ModelRef:  store.Meta.ModelRef,
		StateFile: stateFile,
		Store:     store,
	}
}

// EndByID marks the incident as ended. Persists the timestamp on the
// first call; subsequent calls do not overwrite the original timestamp
// (idempotent) but may still update the verdict when a new non-empty
// one is supplied — letting an agent record a post-hoc conclusion
// without clobbering the time the incident actually closed.
func EndByID(id, verdict string) (*Incident, error) {
	inc, err := LoadByID(id)
	if err != nil {
		return nil, err
	}
	if inc.Store.Meta.Ended.IsZero() {
		inc.Store.Meta.Ended = time.Now()
	} // else: preserve the original close time (idempotent re-end)
	if verdict != "" {
		inc.Store.Meta.Verdict = verdict
	}
	if err := inc.Store.Save(); err != nil {
		return nil, fmt.Errorf("saving end marker: %w", err)
	}
	inc.Ended = inc.Store.Meta.Ended
	inc.Verdict = inc.Store.Meta.Verdict
	return inc, nil
}

// Current reads .mgtt-current and loads the incident state.
func Current() (*Incident, error) {
	data, err := os.ReadFile(currentFile)
	if err != nil {
		return nil, fmt.Errorf("no active incident")
	}
	stateFile := strings.TrimSpace(string(data))
	store, err := facts.Load(stateFile)
	if err != nil {
		return nil, fmt.Errorf("loading state: %w", err)
	}
	return fromStore(store, stateFile), nil
}

// CurrentStoreOrMemory returns the fact store of the active incident or
// a fresh in-memory store when none is active. Most read-only CLI
// commands want exactly this — collapses the four-line pattern scattered
// across plan/status/ls/diagnose into one call.
func CurrentStoreOrMemory() *facts.Store {
	if inc, err := Current(); err == nil && inc.Store != nil {
		return inc.Store
	}
	return facts.NewInMemory()
}

// generateID creates an incident ID from the current UTC timestamp.
// Format: inc-YYYYMMDD-HHMMSS-µµµµµµ-NNN. Microsecond precision plus
// a process-local atomic counter makes collisions effectively
// impossible: two concurrent starts in the same µs get distinct
// counter values.
func generateID() string {
	now := time.Now().UTC()
	seq := idSeq.Add(1) % 1000
	return fmt.Sprintf("inc-%s-%s-%06d-%03d",
		now.Format("20060102"), now.Format("150405"),
		now.Nanosecond()/1000, seq)
}
