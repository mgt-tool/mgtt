...
func TestLoadByID_ReturnsIncidentFromDisk(t *testing.T) {
	...
	_, err := incident.StartIsolated("storefront", "1.0", "inc-persistent-1", "", nil)
	if err != nil {
...
func TestEndByID_MarksEndedAndVerdictInState(t *testing.T) {
	...
	if _, err := incident.StartIsolated("storefront", "1.0", "inc-end-1", "", nil); err != nil {
		t.Fatalf("StartIsolated: %v", err)
	}
...
func TestEndByID_DoesNotTouchCLICurrentPointer(t *testing.T) {
	...
	if _, err := incident.StartIsolated("mcp-model", "1.0", "inc-mcp", "", nil); err != nil {
		t.Fatalf("StartIsolated: %v", err)
	}
