...
func (h *Handler) FactAdd(p FactAddParams) (*FactAddResult, error) {
	...
	return withIncident(p.IncidentID, true, func(inc *incident.Incident) (*FactAddResult, error) {
		f := facts.Fact{
			Key:       p.Key,
			Value:     p.Value,
			Collector: "agent",
...
func (h *Handler) FactsList(p FactsListParams) (*FactsListResult, error) {
	return withIncident(p.IncidentID, false, func(inc *incident.Incident) (*FactsListResult, error) {
