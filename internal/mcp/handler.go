...
//
// Filesystem note: --readonly-only and on-write gate the *probe* layer,
// not model_ref reads. Agents can read any YAML the server process has
// access to; operators who need read confinement must chroot / bind-
// mount the server before exposure.
func (h *Handler) IncidentStart(p IncidentStartParams) (*IncidentStartResult, error) {
	...
	inc, err := incident.StartIsolated(m.Meta.Name, m.Meta.Version, p.ID, absRef, p.Suspect)
	if err != nil {
...
func (h *Handler) IncidentEnd(p IncidentEndParams) (*IncidentEndResult, error) {
	...
	// write lock. Evicting after Unlock would let a concurrent lockFor
	// grab the same mutex, then a third caller allocate a fresh one —
	// two goroutines serialising against different mutexes on the same
