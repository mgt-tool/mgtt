...
func TestLoad_StorefrontModel(t *testing.T) {
	...
	// store healthy=["available == true", "connection_count < 500"]
	wantHealthy := []string{"available == true", "connection_count < 500"}
	if len(store.HealthyRaw) != len(wantHealthy) || store.HealthyRaw[0] != wantHealthy[0] || store.HealthyRaw[1] != wantHealthy[1] {
		t.Errorf("store.HealthyRaw = %v, want %v", store.HealthyRaw, wantHealthy)
	}
...
func TestLoad_HealthyCompiled(t *testing.T) {
	...
	if len(store.Healthy) != 2 {
		t.Fatalf("expected 2 compiled healthy expressions for store, got %d", len(store.Healthy))
	}
}
