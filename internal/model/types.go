...
// EffectiveProviders returns c.Providers, falling back to m.Meta.Providers
// when the component-level list is empty. This is the "whose provider types
// do we consult?" question every caller that resolves a component's type
// asks — keep the fallback in one place.
func (c *Component) EffectiveProviders(m *Model) []string {
...
// mergeVars merges base with override; override wins on key collision.
// Returns nil if both are empty. Keeps nil-tolerance so callers don't
// need to pre-check either map.
