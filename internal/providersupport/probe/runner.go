...
func (r *ExternalRunner) Run(ctx context.Context, cmd Command) (res Result, err error) {
	...
	stdout, err := r.execWithTimeout(ctx, cmd.Timeout, argv)
	if err != nil {
		return Result{}, err
	}
	...
	return r.parseRunnerOutput(stdout)
}

// execWithTimeout runs the runner binary with argv under a bounded
// context, places the child in its own process group so timeout kills
// the whole subtree, and classifies failures into the sentinel error
// taxonomy on return.
func (r *ExternalRunner) execWithTimeout(ctx context.Context, cmdTimeout time.Duration, argv []string) ([]byte, error) {
	timeout := cmdTimeout
	if timeout == 0 {
	...
	// exec.CommandContext only kills the direct child; real provider
	// runners fork kubectl/aws/etc., which would orphan on timeout.
	// Put the child in its own process group so the cancel hook can
	// kill the entire subtree.
	setProcessGroup(c)
	...
	if errors.Is(ctx.Err(), context.DeadlineExceeded) {
		return nil, fmt.Errorf("%w: runner %s exceeded %s", ErrTransient, r.Binary, timeout)
	}
	var exitErr *exec.ExitError
	if errors.As(runErr, &exitErr) {
		return nil, ClassifyExit(exitErr.ExitCode(), stderr.String())
	}
	return nil, fmt.Errorf("%w: runner %s: %w", ErrEnv, r.Binary, runErr)
}

// parseRunnerOutput decodes the runner's JSON stdout and validates
// Status against the whitelist. Missing Status defaults to StatusOk
// (back-compat with pre-1.1 providers).
func (r *ExternalRunner) parseRunnerOutput(stdout []byte) (Result, error) {
	var rr struct {
