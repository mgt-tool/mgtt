...
func TestImageRunner_AppliesCaps(t *testing.T) {
	...
	joined := strings.Join(r.ArgPrefix, " ")
	for _, sub := range []string{"run", "--rm", "/home/alice/.kube:/root/.kube:ro", "/home/alice/.aws:/root/.aws:ro"} {
		if !strings.Contains(joined, sub) {
			t.Errorf("prefix missing %q; got %v", sub, r.ArgPrefix)
		}
	}
