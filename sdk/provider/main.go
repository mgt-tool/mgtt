...
func Run(ctx context.Context, r *Registry, args []string, stdout, stderr io.Writer) int {
	...
	case "probe":
		return runProbe(ctx, r, args, stdout, stderr)
	default:
...
func runProbe(ctx context.Context, r *Registry, args []string, stdout, stderr io.Writer) int {
	if len(args) < 3 {
		fmt.Fprintln(stderr, "probe requires <name> and <fact>")
		return 1
	}
	req, code := parseProbeRequest(args, stderr)
	if code != 0 {
		return code
	}
...
func parseProbeRequest(args []string, stderr io.Writer) (Request, int) {
	req := Request{
		Name:  args[1],
		Fact:  args[2],
